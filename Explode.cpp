...
*   Date: 04 Dec 2013 
*   Description: This file implements the Explode class  
*/
...
//Generate the flare sprites
        if (!this->flareOneTexture.initialize(g, explodeNS::FLARE_ONE_IMAGE))
        ...
        if (!this->flareTwoTexture.initialize(g, explodeNS::FLARE_TWO_IMAGE))
                throw GameError(gameErrorNS::FATAL_ERROR, "Error initializing the explosion flare");
...
//Generate the spark sprites
        if (!this->sparkTexture.initialize(g, explodeNS::SPARK_IMAGE))
...
void Explode::explodeAt(float x, float y) {
        ...
        //Fade out the first flare
                        ...
                        this->flareOne->setX(static_cast<float>(x - explodeNS::FLARE_WIDTH / 2));
                        this->flareOne->setY(static_cast<float>(y - explodeNS::FLARE_HEIGHT / 2));
        ...
        //Fade out the second flare
                        ...
                        this->flareTwo->setX(static_cast<float>(x - explodeNS::FLARE_WIDTH / 2));
                        this->flareTwo->setY(static_cast<float>(y - explodeNS::FLARE_HEIGHT / 2));
...
//Activate more sparks, if needed
        if (this->sparkCounter < explodeNS::SPARK_COUNT) {
                ...
                //Calculate the vectors of each of the sparks
                        deg = static_cast<float>(rand() % 360);
                        xVec = static_cast<float>(explodeNS::SPARK_SPEED_CONST * cos(deg * PI / 180.0f + PI / 2.0f));
                        yVec = static_cast<float>(explodeNS::SPARK_SPEED_CONST * sin(deg * PI / 180.0f + PI / 2.0f));
